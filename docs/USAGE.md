...
Forced restart replaces the supervised child directly. It bypasses OpenClaw's
restart-recovery handoff and can lose in-flight work, so it is intentionally the
emergency path rather than the default.
