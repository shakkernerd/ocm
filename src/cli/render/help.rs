...
pub fn migrate_help(cmd: &str) -> String {
    render_leaf(
        ...
        &[
            ...
            "If the config root, mcp, or mcp.apps is owned by $include, flatten that section before migrating so OCM can reset the origin without changing include ownership.",
    ...
    match action {
        "import" => Some(render_leaf(
            ...
            &[
                ...
                "A copied public MCP app sandbox origin is removed unless --sandbox-origin supplies a dedicated origin for the new env.",
                "If the config root, mcp, or mcp.apps is owned by $include, flatten that section before importing so OCM can reset the origin without changing include ownership.",
    ...
    Some(match action {
        ...
        "clone" => render_leaf(
            ...
            &[
                ...
                "If the config root, mcp, or mcp.apps is owned by $include, flatten that section before cloning so OCM can reset the origin without changing include ownership.",
